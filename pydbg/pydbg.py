...
#            if EXCEPTION_BREAKPOINT==self.exception_code :
#                print 'handle PID:',dbg.dwProcessId
    ...
    ####################################################################################################################
    def disasm (self, address ,process_id=None):
        '''
        ...
        try:
            if process_id==None :
                ...
                data  = self.read_process_memory(address, 32 ,process_id)
        except:
    ...
    ####################################################################################################################
    def disasm_around (self, address, num_inst=5 , disasm_process_id=None):
        '''
        ...
        if num_inst == 0:
            if disasm_process_id==None :
                return [(address, self.disasm(address))]
            return [(address, self.disasm(address,disasm_process_id))]
        ...
        try:
            if disasm_process_id==None :
                ...
                data = self.read_process_memory(address - window_size, window_size * 2,disasm_process_id)
        except:
    ...
    ####################################################################################################################
    def read_process_memory (self, address, length ,read_process_id=None):
        '''
        ...
        @raise pdx: An exception is raisedad on failure.
        @rtype:     Raw
        @return:    Read data.
        sd
        '''
        ...
        while length:
            if read_process_id==None :
                ...
                if not kernel32.ReadProcessMemory(kernel32.OpenProcess(PROCESS_ALL_ACCESS,False,read_process_id), address, read_buf, length, byref(count)):
                    if not len(data):
                        raise pdx("ReadProcessMemory(%08x, %d, read=%d)" % (address, length, count.value), True)
                    else:
                        return data
    ...
    def wait_for_process_exit(self) :
        kernel32.WaitForSingleObject(self.h_process,-1);
